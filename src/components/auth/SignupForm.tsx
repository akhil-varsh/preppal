...
export const SignupForm: React.FC<SignupFormProps> = ({ onToggleMode }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    <Card className="w-full max-w-md mx-auto bg-slate-auth-card border-0 shadow-2xl rounded-2xl">
      <CardHeader>
        <CardTitle className="text-center font-playfair text-2xl text-slate-800">Join PrepPal</CardTitle>
        <p className="text-center text-slate-600 font-nunito">Create your account and start practicing</p>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <Input
              ...
              required
              disabled={loading}
              icon={<User className="h-4 w-4 slate-input-icon" />}
